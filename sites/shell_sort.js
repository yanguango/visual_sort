// Generated by js_of_ocaml 2.2
(function(V){"use strict";var
S=254,B=255,aB=250,D=1073741823,C=200,p=400,aF=390,j="",aD="canvas",R=" : file already exists",aA="gray",Q="/",aE="fd ",aC="index out of bounds";function
aK(a,b){throw[0,a,b]}function
W(a){if(a.charCodeAt(a.length-1)==10)a=a.substr(0,a.length-1);var
b=V.console;b&&b.error&&b.error(a)}var
g=[0];function
U(a,b){if(!a)return j;if(a&1)return U(a-1,b)+b;var
c=U(a>>1,b);return c+c}function
h(a){if(a!=null){this.bytes=this.fullBytes=a;this.last=this.len=a.length}}function
aM(){aK(g[4],new
h(aC))}h.prototype={string:null,bytes:null,fullBytes:null,array:null,len:null,last:0,toJsString:function(){var
a=this.getFullBytes();try{return this.string=decodeURIComponent(escape(a))}catch(f){W('MlString.toJsString: wrong encoding for "%s" ',a);return a}},toBytes:function(){if(this.string!=null)try{var
a=unescape(encodeURIComponent(this.string))}catch(f){W('MlString.toBytes: wrong encoding for "%s" ',this.string);var
a=this.string}else{var
a=j,c=this.array,d=c.length;for(var
b=0;b<d;b++)a+=String.fromCharCode(c[b])}this.bytes=this.fullBytes=a;this.last=this.len=a.length;return a},getBytes:function(){var
a=this.bytes;if(a==null)a=this.toBytes();return a},getFullBytes:function(){var
a=this.fullBytes;if(a!==null)return a;a=this.bytes;if(a==null)a=this.toBytes();if(this.last<this.len){this.bytes=a+=U(this.len-this.last,"\0");this.last=this.len}this.fullBytes=a;return a},toArray:function(){var
c=this.bytes;if(c==null)c=this.toBytes();var
b=[],d=this.last;for(var
a=0;a<d;a++)b[a]=c.charCodeAt(a);for(d=this.len;a<d;a++)b[a]=0;this.string=this.bytes=this.fullBytes=null;this.last=this.len;this.array=b;return b},getArray:function(){var
a=this.array;if(!a)a=this.toArray();return a},getLen:function(){var
a=this.len;if(a!==null)return a;this.toBytes();return this.len},toString:function(){var
a=this.string;return a?a:this.toJsString()},valueOf:function(){var
a=this.string;return a?a:this.toJsString()},blitToArray:function(a,b,c,d){var
g=this.array;if(g)if(c<=a)for(var
e=0;e<d;e++)b[c+e]=g[a+e];else
for(var
e=d-1;e>=0;e--)b[c+e]=g[a+e];else{var
f=this.bytes;if(f==null)f=this.toBytes();var
h=this.last-a;if(d<=h)for(var
e=0;e<d;e++)b[c+e]=f.charCodeAt(a+e);else{for(var
e=0;e<h;e++)b[c+e]=f.charCodeAt(a+e);for(;e<d;e++)b[c+e]=0}}},get:function(a){var
c=this.array;if(c)return c[a];var
b=this.bytes;if(b==null)b=this.toBytes();return a<this.last?b.charCodeAt(a):0},safeGet:function(a){if(this.len==null)this.toBytes();if(a<0||a>=this.len)aM();return this.get(a)},set:function(a,b){var
c=this.array;if(!c){if(this.last==a){this.bytes+=String.fromCharCode(b&B);this.last++;return 0}c=this.toArray()}else
if(this.bytes!=null)this.bytes=this.fullBytes=this.string=null;c[a]=b&B;return 0},safeSet:function(a,b){if(this.len==null)this.toBytes();if(a<0||a>=this.len)aM();this.set(a,b)},fill:function(a,b,c){if(a>=this.last&&this.last&&c==0)return;var
d=this.array;if(!d)d=this.toArray();else
if(this.bytes!=null)this.bytes=this.fullBytes=this.string=null;var
f=a+b;for(var
e=a;e<f;e++)d[e]=c},compare:function(a){if(this.string!=null&&a.string!=null){if(this.string<a.string)return-1;if(this.string>a.string)return 1;return 0}var
b=this.getFullBytes(),c=a.getFullBytes();if(b<c)return-1;if(b>c)return 1;return 0},equal:function(a){if(this.string!=null&&a.string!=null)return this.string==a.string;return this.getFullBytes()==a.getFullBytes()},lessThan:function(a){if(this.string!=null&&a.string!=null)return this.string<a.string;return this.getFullBytes()<a.getFullBytes()},lessEqual:function(a){if(this.string!=null&&a.string!=null)return this.string<=a.string;return this.getFullBytes()<=a.getFullBytes()}};function
E(a){this.string=a}E.prototype=new
h();function
aL(a,b){aK(a,new
E(b))}function
F(a){aL(g[4],a)}function
aI(){F(aC)}function
bb(a,b){if(b<0||b>=a.length-1)aI();return a[b+1]}function
bc(a,b,c){if(b<0||b>=a.length-1)aI();a[b+1]=c;return 0}function
bd(a,b,c,d,e){if(e===0)return;if(a.array!=null&&c.last==0&&d==0&&e==c.len){c.array=a.array.slice(b,b+e);c.bytes=c.string=null;return}if(d===c.last&&c.bytes!=null){var
f=a.bytes;if(f==null)f=a.toBytes();if(b>0||a.last>e)f=f.slice(b,b+e);c.bytes+=f;c.last+=f.length;return}var
g=c.array;if(!g)g=c.toArray();else
c.bytes=c.string=null;a.blitToArray(b,g,d,e)}function
v(c,b){if(c.fun)return v(c.fun,b);var
a=c.length,d=a-b.length;if(d==0)return c.apply(null,b);else
if(d<0)return v(c.apply(null,b.slice(0,a)),b.slice(a));else
return function(a){return v(c,b.concat([a]))}}function
aG(a){this.bytes=j;this.len=a}aG.prototype=new
h();function
bf(a){if(a<0)F("String.create");return new
aG(a)}function
bj(a,b){var
c=a[3]<<16,d=b[3]<<16;if(c>d)return 1;if(c<d)return-1;if(a[2]>b[2])return 1;if(a[2]<b[2])return-1;if(a[1]>b[1])return 1;if(a[1]<b[1])return-1;return 0}function
bk(a,b){if(a<b)return-1;if(a==b)return 0;return 1}function
be(a,b,c){var
e=[];for(;;){if(!(c&&a===b))if(a
instanceof
h)if(b
instanceof
h){if(a!==b){var
d=a.compare(b);if(d!=0)return d}}else
return 1;else
if(a
instanceof
Array&&a[0]===(a[0]|0)){var
f=a[0];if(f===S)f=0;if(f===aB){a=a[1];continue}else
if(b
instanceof
Array&&b[0]===(b[0]|0)){var
g=b[0];if(g===S)g=0;if(g===aB){b=b[1];continue}else
if(f!=g)return f<g?-1:1;else
switch(f){case
248:var
d=bk(a[2],b[2]);if(d!=0)return d;break;case
251:F("equal: abstract value");case
B:var
d=bj(a,b);if(d!=0)return d;break;default:if(a.length!=b.length)return a.length<b.length?-1:1;if(a.length>1)e.push(a,b,1)}}else
return 1}else
if(b
instanceof
h||b
instanceof
Array&&b[0]===(b[0]|0))return-1;else
if(typeof
a!="number"&&a&&a.compare)return a.compare(b,c);else{if(a<b)return-1;if(a>b)return 1;if(a!=b){if(!c)return NaN;if(a==a)return 1;if(b==b)return-1}}if(e.length==0)return 0;var
i=e.pop();b=e.pop();a=e.pop();if(i+1<a.length)e.push(a,b,i+1);a=a[i];b=b[i]}}function
bi(a,b){return+(be(a,b,false)>=0)}function
bl(a){var
c=Array.prototype.slice;return function(){var
b=arguments.length>0?c.call(arguments):[undefined];return v(a,b)}}function
bm(a,b){var
d=[0];for(var
c=1;c<=a;c++)d[c]=b;return d}function
aH(a){var
b=a.length;this.array=a;this.len=this.last=b}aH.prototype=new
h();var
bn=function(){function
m(a,b){return a+b|0}function
l(a,b,c,d,e,f){b=m(m(b,a),m(d,f));return m(b<<e|b>>>32-e,c)}function
h(a,b,c,d,e,f,g){return l(b&c|~b&d,a,b,e,f,g)}function
i(a,b,c,d,e,f,g){return l(b&d|c&~d,a,b,e,f,g)}function
j(a,b,c,d,e,f,g){return l(b^c^d,a,b,e,f,g)}function
k(a,b,c,d,e,f,g){return l(c^(b|~d),a,b,e,f,g)}function
n(a,b){var
g=b;a[g>>2]|=128<<8*(g&3);for(g=(g&~3)+8;(g&63)<60;g+=4)a[(g>>2)-1]=0;a[(g>>2)-1]=b<<3;a[g>>2]=b>>29&536870911;var
l=[1732584193,4023233417,2562383102,271733878];for(g=0;g<a.length;g+=16){var
c=l[0],d=l[1],e=l[2],f=l[3];c=h(c,d,e,f,a[g+0],7,3614090360);f=h(f,c,d,e,a[g+1],12,3905402710);e=h(e,f,c,d,a[g+2],17,606105819);d=h(d,e,f,c,a[g+3],22,3250441966);c=h(c,d,e,f,a[g+4],7,4118548399);f=h(f,c,d,e,a[g+5],12,1200080426);e=h(e,f,c,d,a[g+6],17,2821735955);d=h(d,e,f,c,a[g+7],22,4249261313);c=h(c,d,e,f,a[g+8],7,1770035416);f=h(f,c,d,e,a[g+9],12,2336552879);e=h(e,f,c,d,a[g+10],17,4294925233);d=h(d,e,f,c,a[g+11],22,2304563134);c=h(c,d,e,f,a[g+12],7,1804603682);f=h(f,c,d,e,a[g+13],12,4254626195);e=h(e,f,c,d,a[g+14],17,2792965006);d=h(d,e,f,c,a[g+15],22,1236535329);c=i(c,d,e,f,a[g+1],5,4129170786);f=i(f,c,d,e,a[g+6],9,3225465664);e=i(e,f,c,d,a[g+11],14,643717713);d=i(d,e,f,c,a[g+0],20,3921069994);c=i(c,d,e,f,a[g+5],5,3593408605);f=i(f,c,d,e,a[g+10],9,38016083);e=i(e,f,c,d,a[g+15],14,3634488961);d=i(d,e,f,c,a[g+4],20,3889429448);c=i(c,d,e,f,a[g+9],5,568446438);f=i(f,c,d,e,a[g+14],9,3275163606);e=i(e,f,c,d,a[g+3],14,4107603335);d=i(d,e,f,c,a[g+8],20,1163531501);c=i(c,d,e,f,a[g+13],5,2850285829);f=i(f,c,d,e,a[g+2],9,4243563512);e=i(e,f,c,d,a[g+7],14,1735328473);d=i(d,e,f,c,a[g+12],20,2368359562);c=j(c,d,e,f,a[g+5],4,4294588738);f=j(f,c,d,e,a[g+8],11,2272392833);e=j(e,f,c,d,a[g+11],16,1839030562);d=j(d,e,f,c,a[g+14],23,4259657740);c=j(c,d,e,f,a[g+1],4,2763975236);f=j(f,c,d,e,a[g+4],11,1272893353);e=j(e,f,c,d,a[g+7],16,4139469664);d=j(d,e,f,c,a[g+10],23,3200236656);c=j(c,d,e,f,a[g+13],4,681279174);f=j(f,c,d,e,a[g+0],11,3936430074);e=j(e,f,c,d,a[g+3],16,3572445317);d=j(d,e,f,c,a[g+6],23,76029189);c=j(c,d,e,f,a[g+9],4,3654602809);f=j(f,c,d,e,a[g+12],11,3873151461);e=j(e,f,c,d,a[g+15],16,530742520);d=j(d,e,f,c,a[g+2],23,3299628645);c=k(c,d,e,f,a[g+0],6,4096336452);f=k(f,c,d,e,a[g+7],10,1126891415);e=k(e,f,c,d,a[g+14],15,2878612391);d=k(d,e,f,c,a[g+5],21,4237533241);c=k(c,d,e,f,a[g+12],6,1700485571);f=k(f,c,d,e,a[g+3],10,2399980690);e=k(e,f,c,d,a[g+10],15,4293915773);d=k(d,e,f,c,a[g+1],21,2240044497);c=k(c,d,e,f,a[g+8],6,1873313359);f=k(f,c,d,e,a[g+15],10,4264355552);e=k(e,f,c,d,a[g+6],15,2734768916);d=k(d,e,f,c,a[g+13],21,1309151649);c=k(c,d,e,f,a[g+4],6,4149444226);f=k(f,c,d,e,a[g+11],10,3174756917);e=k(e,f,c,d,a[g+2],15,718787259);d=k(d,e,f,c,a[g+9],21,3951481745);l[0]=m(c,l[0]);l[1]=m(d,l[1]);l[2]=m(e,l[2]);l[3]=m(f,l[3])}var
o=[];for(var
g=0;g<4;g++)for(var
n=0;n<4;n++)o[g*4+n]=l[g]>>8*n&B;return o}return function(a,b,c){var
h=[];if(a.array){var
f=a.array;for(var
d=0;d<c;d+=4){var
e=d+b;h[d>>2]=f[e]|f[e+1]<<8|f[e+2]<<16|f[e+3]<<24}for(;d<c;d++)h[d>>2]|=f[d+b]<<8*(d&3)}else{var
g=a.getFullBytes();for(var
d=0;d<c;d+=4){var
e=d+b;h[d>>2]=g.charCodeAt(e)|g.charCodeAt(e+1)<<8|g.charCodeAt(e+2)<<16|g.charCodeAt(e+3)<<24}for(;d<c;d++)h[d>>2]|=g.charCodeAt(d+b)<<8*(d&3)}return new
aH(n(h,c))}}();function
k(a){aL(g[2],a)}function
bo(a){if(!a.opened)k("Cannot flush a closed channel");if(a.buffer==j)return 0;if(a.output)switch(a.output.length){case
2:a.output(a,a.buffer);break;default:a.output(a.buffer)}a.buffer=j}function
aJ(a){a=a
instanceof
h?a.toString():a;k(a+": No such file or directory")}var
bg=Q;function
G(a){a=a
instanceof
h?a.toString():a;if(a.charCodeAt(0)!=47)a=bg+a;var
d=a.split(Q),b=[];for(var
c=0;c<d.length;c++)switch(d[c]){case"..":if(b.length>1)b.pop();break;case".":case
j:if(b.length==0)b.push(j);break;default:b.push(d[c]);break}b.orig=a;return b}function
l(){this.content={}}l.prototype={exists:function(a){return this.content[a]?1:0},mk:function(a,b){this.content[a]=b},get:function(a){return this.content[a]},list:function(){var
a=[];for(var
b
in
this.content)a.push(b);return a},remove:function(a){delete
this.content[a]}};var
I=new
l();I.mk(j,new
l());function
T(a){var
b=I;for(var
c=0;c<a.length;c++){if(!(b.exists&&b.exists(a[c])))aJ(a.orig);b=b.get(a[c])}return b}function
bC(a){var
c=G(a),b=T(c);return b
instanceof
l?1:0}function
u(a){this.data=a}u.prototype={content:function(){return this.data},truncate:function(){this.data.length=0}};function
bh(a,b){var
e=G(a),c=I;for(var
f=0;f<e.length-1;f++){var
d=e[f];if(!c.exists(d))c.mk(d,new
l());c=c.get(d);if(!(c
instanceof
l))k(e.orig+R)}var
d=e[e.length-1];if(c.exists(d))k(e.orig+R);if(b
instanceof
l)c.mk(d,b);else
if(b
instanceof
u)c.mk(d,b);else
if(b
instanceof
h)c.mk(d,new
u(b.getArray()));else
if(b
instanceof
Array)c.mk(d,new
u(b));else
if(b.toString)c.mk(d,new
u(new
h(b.toString()).getArray()));else
F("caml_fs_register")}function
bB(a){var
b=I,d=G(a),e;for(var
c=0;c<d.length;c++){if(b.auto)e=b.auto;if(!(b.exists&&b.exists(d[c])))return e?e(d.join(Q)):0;b=b.get(d[c])}return 1}function
w(a,b,c){if(g.fds===undefined)g.fds=new
Array();c=c?c:{};var
d={};d.array=b;d.offset=c.append?d.array.length:0;d.flags=c;g.fds[a]=d;g.fd_last_idx=a;return a}function
bF(a,b,c){var
d={};while(b){switch(b[1]){case
0:d.rdonly=1;break;case
1:d.wronly=1;break;case
2:d.append=1;break;case
3:d.create=1;break;case
4:d.truncate=1;break;case
5:d.excl=1;break;case
6:d.binary=1;break;case
7:d.text=1;break;case
8:d.nonblock=1;break}b=b[2]}var
f=a.toString(),i=G(a);if(d.rdonly&&d.wronly)k(f+" : flags Open_rdonly and Open_wronly are not compatible");if(d.text&&d.binary)k(f+" : flags Open_text and Open_binary are not compatible");if(bB(a)){if(bC(a))k(f+" : is a directory");if(d.create&&d.excl)k(f+R);var
h=g.fd_last_idx?g.fd_last_idx:0,e=T(i);if(d.truncate)e.truncate();return w(h+1,e.content(),d)}else
if(d.create){var
h=g.fd_last_idx?g.fd_last_idx:0;bh(a,[]);var
e=T(i);return w(h+1,e.content(),d)}else
aJ(a)}w(0,[]);w(1,[]);w(2,[]);function
bp(a){var
b=g.fds[a];if(b.flags.wronly)k(aE+a+" is writeonly");return{data:b,fd:a,opened:true}}function
bE(a){if(a.charCodeAt(a.length-1)==10)a=a.substr(0,a.length-1);var
b=V.console;b&&b.log&&b.log(a)}var
H=new
Array();function
bz(a,b){var
e=new
h(b),d=e.getLen();for(var
c=0;c<d;c++)a.data.array[a.data.offset+c]=e.get(c);a.data.offset+=d;return 0}function
bq(a){var
b;switch(a){case
1:b=bE;break;case
2:b=W;break;default:b=bz}var
d=g.fds[a];if(d.flags.rdonly)k(aE+a+" is readonly");var
c={data:d,fd:a,opened:true,buffer:j,output:b};H[c.fd]=c;return c}function
br(){var
a=0;for(var
b
in
H)if(H[b].opened)a=[0,H[b],a];return a}function
bv(a){throw[0,a]}function
bw(){bv(g[6])}function
bs(a,b){if(b==0)bw();return a%b}function
bu(a){return new
h(a)}function
bx(a,b){g[a+1]=b}var
bt={};function
by(a,b){bt[a.toString()]=b;return 0}function
bA(){return 32}function
bD(){var
a=new
Date()^4294967295*Math.random();return{valueOf:function(){return a},0:0,1:a,length:2}}var
f=bb,P=bc,ay=bd,az=bm,aw=bq,b=bu,a=bx,ax=by,X=[0,b("Invalid_argument")],ae=[0,b("Assert_failure")],au=[0,1];a(11,[0,b("Undefined_recursive_module")]);a(10,ae);a(9,[0,b("Sys_blocked_io")]);a(8,[0,b("Stack_overflow")]);a(7,[0,b("Match_failure")]);a(6,[0,b("Not_found")]);a(5,[0,b("Division_by_zero")]);a(4,[0,b("End_of_file")]);a(3,X);a(2,[0,b("Failure")]);a(1,[0,b("Sys_error")]);a(0,[0,b("Out_of_memory")]);var
aN=b("Pervasives.do_at_exit"),a0=b("Random.int"),aU=b("x"),a1=[0,987910699,495797812,364182224,414272206,318284740,990407751,383018966,270373319,840823159,24560019,536292337,512266505,189156120,730249596,143776328,51606627,140166561,366354223,1003410265,700563762,981890670,913149062,526082594,1021425055,784300257,667753350,630144451,949649812,48546892,415514493,258888527,511570777,89983870,283659902,308386020,242688715,482270760,865188196,1027664170,207196989,193777847,619708188,671350186,149669678,257044018,87658204,558145612,183450813,28133145,901332182,710253903,510646120,652377910,409934019,801085050],a5=b("Js.Error"),a6=b("jsError"),a9=b("Dom_html.Canvas_not_available"),ba=[0,b("shell_sort.ml"),36,17];function
Z(a){return b(j+a)}function
_(a,b){if(a){var
c=a[1];return[0,c,_(a[2],b)]}return b}bp(0);aw(1);aw(2);function
$(a){var
b=br(0);for(;;){if(b){var
c=b[2],d=b[1];try{bo(d)}catch(f){}var
b=c;continue}return 0}}ax(aN,$);var
af=[0,0],aS=bA(0);function
ag(a){af[1]=[0,a,af[1]];return 0}32===aS;var
c=[0,a1.slice(),0],i=V,an=null,ao=[0,a5],a4=i.Array;ax(a6,[0,ao,{}][0+1]);var
a2=undefined,a3=false;ag(function(a){return a[1]===ao?[0,new
E(a[2].toString())]:0});ag(function(a){return a
instanceof
a4?0:[0,new
E(a.toString())]});var
a7="2d",a8=i.document,a_=[0,a9];i.HTMLElement===a2;var
am=bD(0),a$=20,N=20,ai=0===am.length-1?[0,0]:am,L=ai.length-1,aT=0;if(!0){var
s=aT;for(;;){P(c[1],s,s);var
aZ=s+1|0;if(54!==s){var
s=aZ;continue}break}}var
M=[0,aU],aV=0,aW=55,aX=bi(55,L)?aW:L,aj=54+aX|0;if(!(aj<0)){var
r=aV;for(;;){var
ak=r%55|0,al=M[1],ah=Z(f(ai,bs(r,L))),J=al.getLen(),Y=ah.getLen(),x=bf(J+Y|0);ay(al,0,x,0,J);ay(ah,0,x,J,Y);M[1]=bn(x,0,x.getLen());var
y=M[1];P(c[1],ak,(f(c[1],ak)^(((y.safeGet(0)+(y.safeGet(1)<<8)|0)+(y.safeGet(2)<<16)|0)+(y.safeGet(3)<<24)|0))&D);var
aY=r+1|0;if(aj!==r){var
r=aY;continue}break}}c[2]=0;function
ap(a){var
d=99;if(!(D<99))if(0<d)for(;;){c[2]=(c[2]+1|0)%55|0;var
e=f(c[1],c[2]),b=(f(c[1],(c[2]+24|0)%55|0)+(e^e>>>25&31)|0)&D;P(c[1],c[2],b);var
g=b%d|0;if(((D-d|0)+1|0)<(b-g|0))continue;return g+1|0}throw[0,X,a0]}if(0===N)var
n=[0];else{var
aa=az(N,ap(0)),ab=N-1|0,aO=1;if(!(ab<1)){var
q=aO;for(;;){aa[q+1]=ap(q);var
aP=q+1|0;if(ab!==q){var
q=aP;continue}break}}var
n=aa}function
aq(a,b){return[0,b,[0,[S,a*40,aF-b*3],a$,b*3],0]}var
K=n.length-1;if(0===K)var
d=[0];else{var
ac=az(K,aq(0,n[0+1])),ad=K-1|0,aQ=1;if(!(ad<1)){var
m=aQ;for(;;){ac[m+1]=aq(m,n[m+1]);var
aR=m+1|0;if(ad!==m){var
m=aR;continue}break}}var
d=ac}var
t=i.document.createElement(aD);if(1-(t.getContext==an?1:0)){t[b("width")]=800;t[b("height")]=p;var
e=t.getContext(a7),O=function(a,b){a[1]=b;a[2][1][2]=aF-b*3;a[2][3]=b*3;return 0},ar=function(a,b,c){var
d=f(a,b)[1];O(f(a,b),f(a,c)[1]);return O(f(a,c),d)},z=function(a,b){a[1]=_(a[1],[0,b,0]);return 0},A=function(a,b){var
d=f(a,b),c=d[2],g=d[1],h=d[3];return function(a){e.clearRect(c[1][1],c[1][2],c[2],p);e.fillStyle=h?"black":aA;e.fillRect(c[1][1],c[1][2],c[2],c[3]);e.fillText(g,c[1][1]+3,p);return 0}},as=function(a,b,c){var
h=c?c[1]:1,g=f(a,b),d=g[2],i=g[1];e.clearRect(d[1][1],0,d[2],p);if(h)e.fillStyle="yellow";e.fillRect(d[1][1],d[1][2],d[2],d[3]);return e.fillText(i,d[1][1]+3,p)},at=function(a){if(a){var
c=a[2];(function(a,b){return a.length==1?a(b):v(a,[b])}(a[1],0));i.setTimeout(function(a){return at(c)},C);var
b=0}else
var
b=a;return b},o=[0,0],av=function(a){var
G=a8.getElementById(aD);if(G==an)throw[0,ae,ba];G.appendChild(t);var
y=d.length-1-1|0,H=0;if(!(y<0)){var
s=H;for(;;){var
B=d[s+1],q=B[2],L=B[1];e.fillStyle=aA;e.fillRect(q[1][1],q[1][2],q[2],q[3]);e.fillText(L,q[1][1]+3,p);var
I=s+1|0;if(y!==s){var
s=I;continue}break}}var
E=d.length-1,u=1;for(;;){if(u<(E/3|0)){var
u=(3*u|0)+1|0;continue}var
h=u;for(;;){if(1<=h){var
F=h-1|0,M=0;if(!(F<0)){var
v=M;a:for(;;){var
l=v;b:for(;;){if(l<E){var
k=f(d,l)[2];z(o,function(k){return function(a){e.fillStyle="red";e.fillRect(k[1][1],k[1][2],k[2],k[3]);return 0}}(k));var
g=l;for(;;){if(0<=(g-h|0)){if(f(d,g)[1]<f(d,g-h|0)[1]){var
b=g-h|0,j=f(d,b)[2];z(o,function(j,b){return function(a){e.fillStyle="green";e.fillRect(j[1][1],j[1][2],j[2],j[3]);i.setTimeout(A(d,b),C);return 0}}(j,b));var
c=g-h|0,r=au?au[1]:1;z(o,function(r,g,c){return function(a){ar(d,g,c);as(d,g,[0,r]);as(d,c,[0,r]);if(r){i.setTimeout(A(d,g),C);i.setTimeout(A(d,c),C)}return 0}}(r,g,c));ar(d,g,g-h|0)}var
g=g-h|0;continue}z(o,A(d,l));var
l=l+h|0;continue b}}var
N=v+1|0;if(F!==v){var
v=N;continue a}break}break}}var
h=h/3|0;continue}var
D=n.length-1-1|0,J=0;if(!(D<0)){var
m=J;for(;;){O(f(d,m),n[m+1]);var
K=m+1|0;if(D!==m){var
m=K;continue}break}}var
x=0,w=o[1];for(;;){if(w){var
x=x+1|0,w=w[2];continue}i.alert(Z(x).toString());at(o[1]);return a3}}}};i.onload=bl(function(a){if(a){var
d=av(a);if(!(d|0))a.preventDefault();return d}var
c=event,b=av(c);if(!(b|0))c.returnValue=b;return b});$(0);return}throw[0,a_]}(function(){return this}()));
